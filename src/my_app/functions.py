    ...
    if 'user' in params and 'query' in params:
        ...
        if 'filters' in params:
            ...
            if qdrant_filters_dict:
                item_ids, item_metadata = vector_store_search(query, filters=qdrant_filters_dict)
                recommended_items = recommend_given_items(uid_series, item_ids)
                matched = True
            else:
                # even if filters have been generated by the assistant, no one of them corresponds
                # to labels that are included in the dataset. For this reason, we still want a
                # vector store search, but without filters
