# src/api/entities/services/event_handler_service.py
...
# Assuming entities_sdk.src.entities.services.logging_service defines LoggingUtility
    ...
    def start(self, run_id: str):
        ...
        try:
            ...
            if not run_object:
                 ...
                 logger.warning(f"Cannot start monitoring: Run {run_id} not found via internal client.")
        ...
        try:
            ...
            logger.error(f"Error calling _event_handler.start_monitoring for run {run_id}: {e}", exc_info=True)
    ...
    async def _handle_event(self, event_type: str, event_data: dict):
        ...
        if self._external_callback:
            ...
            if event_type in ["tool_invoked", "action_required"]: # Add other types if necessary
                try:
                    logger.info(f"Executing external synchronous callback for event '{event_type}' for run {run_id or 'Unknown'}.")
                    self._external_callback(event_type, event_data)
                except Exception as e:
                    logger.error(f"Error executing external callback for run {run_id or 'Unknown'}, event {event_type}: {e}", exc_info=True)
