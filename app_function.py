...
def create_db():
    ...
    cursor.execute('''CREATE TABLE IF NOT EXISTS movies_ml (item_id INTEGER PRIMARY KEY, title TEXT, release_date INTEGER, genres TEXT)''')
    ...
    with open('./data/recsys/ml-100k/ml-100k.item', 'r', encoding='utf-8') as f:
        ...
        for line in f:
            ...
            cursor.execute('INSERT OR IGNORE INTO movies_ml VALUES (?, ?, ?, ?)',
...
def define_query(filters):
    ...
    if 'genres' in filters:
        ...
        for genre in genres:
            query_parts.append(f"LOWER(genres) LIKE '%{genre}%'")
    ...
    if 'release_date' in filters:
        ...
        if request is not None:
            query_parts.append(f"release_date > {release_date}") if request == "higher" else query_parts.append(f"release_date < {release_date}")
        else:
            query_parts.append(f"release_date = {release_date}")

    return f"SELECT item_id FROM movies_ml WHERE {'AND '.join(query_parts)}"


def get_interacted_items(user):
    conn = sqlite3.connect('movies.db')
    cursor = conn.cursor()

    sql_query = f'SELECT items FROM interactions WHERE user_id = {user}'
...
def get_top_k_recommendations(params):
    if 'user' in params and 'k' in params:
        ...
        if 'filters' in params:
            ...
            sql_query = define_query(filters)
            conn = sqlite3.connect('movies.db')
            cursor = conn.cursor()
            cursor.execute(sql_query)
        ...
        response_dict = {}
        for item_id in external_item_list:
            response_dict[item_id] = get_item_metadata(params={'item': int(item_id), 'specification': ["title", "release_date", "genres"]}, return_dict=True)

        # get items interacted by user ID
        item_ids = get_interacted_items(user)
        # get metadata of interacted items
        interaction_dict = {}
        for item_id in item_ids.split(','):
            interaction_dict[item_id] = get_item_metadata(params={'item': int(item_id), 'specification': ["title", "release_date", "genres"]}, return_dict=True)
...
def get_item_metadata(params, return_dict=False):
    if 'item' in params and 'specification' in params:
        conn = sqlite3.connect('movies.db')
        cursor = conn.cursor()
        item_id = params.get('item')
        specification = params.get('specification')

        sql_query = f'SELECT {", ".join(specification)} FROM movies_ml WHERE item_id = {item_id}'
        ...
        if result and not return_dict:
            ...
            for i, spec in enumerate(specification):
                return_str += f"{spec}: {result[i]}\n"
            return json.dumps({
                "status": "success",
                "message": f"This is the requested metadata for item {item_id}:\n{return_str}",
            })
        elif result and return_dict:
            r_dict = {}
            for i, spec in enumerate(specification):
                r_dict[spec] = result[i]
            return r_dict
        else:
            if not return_dict:
                return json.dumps({
...
associate_tool_to_assistant(assistant.id, tool=METADATA)
